...
//        public static string ERRORS =                       PREFIX + "files\\errors.txt";
...
//
//
//        public static double MONEY_START = 50000;
...
//        }
//        public static List<Share> getSharesInfo(string strSymbol, int nNumOfDays, DateTime dtFirstDate)
//        {
...
//                string html = getHTTP(string.Format("https://finance.yahoo.com/quote/{0}/history?", strSymbol)).Result;
...
//                    if (date <= dtFirstDate)
//                    {
//                        for (int nIndex = 0; nIndex < nNumOfDays; nIndex++)
//                        {
//                            try
//                            {
...
//        {
//            string html = await getHTTP(string.Format("https://finance.yahoo.com/quote/{0}", strSymbol));
//
//            Share s = new Share();
//            s.close = new Price(
...
//        }
//
//        public void predictTomorrowSymbols(DateTime TODAY)
//        {
//            int nThreads = 0;
...
//            try
//            {
//                var responseString = client.GetStringAsync(string.Format("https://www.zacks.com/includes/classes/z2_class_calendarfunctions_data.php?calltype=eventscal&date={0}&type=1",
...
//                            try
//                            {
//                                window = Share.getSharesInfo(symbol, Consts.WINDOW, new DateTime(TODAY.Year, TODAY.Month, TODAY.Day));
//                                double windowReturn = window[0].close.price / window[window.Count - 1].open.price;
//                                int windowDirection = windowReturn >= 1 ? 1 : -1;
//                                double avgVolume = window.Average(s => s.volume);
//
//                                if (nOffset == 0)
//                                {
//                                    if (window[0].volume >= Consts.MINIMUM_VOLUME)
...
//                                File.AppendAllText(Consts.ERRORS, symbol + "\n");
//                            }
//                            finally
//                            {
...
//                        try
//                        {
//                            List<Share> window = Share.getSharesInfo(symbol, Consts.WINDOW, new DateTime(TODAY.Year, TODAY.Month, TODAY.Day));
//                            double windowReturn = window[0].close.price / window[window.Count - 1].open.price;
//                            int windowDirection = windowReturn >= 1 ? 1 : -1;
//                            double avgVolume = window.Average(s => s.volume);
//
...
//            }
//        }
//        public void minimizePriceWindowAndQuantiles(bool bWitQuantilesCheck, bool bWithWindowReturnsCheck, double minPirce, double upperThanReturn)
//        {
...
//        public double MONEY = Consts.MONEY_START;
...
//            while (true)
//            {
//                Console.WriteLine(TODAY.ToShortDateString() + (recommendationForTommorow.symbols.Count > 0 ? " (" + recommendationForTommorow.symbols.Count  + ")" : ""));
//
//
//                // Get todays returns
...
//                recommendationForTommorow.predictTomorrowSymbols(TODAY);
//                recommendationForTommorow.minimizePriceWindowAndQuantiles(Consts.WITH_QUANTILES_CHECK, Consts.WITH_WINDOW_RETURNS_CHECK, Consts.MINIMUM_PRICE, Consts.MINIMUM_WINDOW_RETURN);
...
//                    try
//                    {
//                        Share s = Share.getSharesInfo(strCurrSymbol, 1, today)[0];
...
//
//            Parallel.ForEach(symbols.Values, (curr) => { 
//            
//                Share s = Share.getRealTimeData(curr.symbol).Result;
...
//                    double dPositionsStartValue = currStrategy.MONEY * dRatio;
//                    int nCountOfShares = (int)(dPositionsStartValue / currShare.open.price) + 1;
//
//                    double change = ((currShare.close.price / currShare.open.price) * 100 - 100) * currShare.direction;
...
//                    last[i] > Consts.MONEY_START ? "green" : "red",
